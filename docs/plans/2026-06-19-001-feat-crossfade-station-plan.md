...
- KTD-6. **Name-leak guard before every generation.** Because Suno blocks real names and the graph stores them, the harness validates each DJ-returned brief before sending to suno-api. It checks the brief's `tags`/`prompt`/`title` against the **real names of the specific nodes feeding that song** (its lineage set), not against the whole inventory, using normalized **word-boundary** matching (reusing `normalize()` from KTD-5) — so case and spacing variants ("band alpha", "BAND ALPHA") are caught, and a name only matches when it appears as a *whole word*, so substring overlaps don't trip it ("Kiss" won't fire on "kisses", "Air" won't fire on "airy"). Note this is whole-word, not meaning-aware: a band whose name is itself a common standalone word ("War", "Yes") *will* match that exact word in ordinary prose ("war drums"), erring toward rejection — tolerable because the guard is scoped to the song's own lineage, so it only bites when such a band is actually a seed. A match rejects the brief back to the DJ for a rewrite, bounded to a small retry count (default 2); after that the brief is dropped from the batch with a logged reason rather than looping forever. This makes AE2 a harness invariant, not a DJ promise.
...
- **Test scenarios:**
  - `Covers AE2.` A brief whose `tags`/`prompt`/`title` contains a lineage-node real name (any case/spacing variant) is rejected by the guard and not sent to suno-api; a rewritten descriptor-only brief passes; a name that appears only as a substring of a longer word (e.g. "Kiss" inside "kisses") does NOT false-trigger.
